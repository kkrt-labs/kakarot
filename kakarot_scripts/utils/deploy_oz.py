...
import json
import boto3
from kakarot_scripts.utils.starknet import deploy_starknet_account, declare
...
def save_accounts_to_json(accounts_data, filename='kakarot_scripts/data/oz_accounts.json'):
...
def get_private_key(private_key):
    ...
    if private_key == "":
        try:
            ...
            raise Exception(f"Secret {secret_name} not found. Please ensure the private key is stored in AWS Secrets Manager.")
    else:
        try:
            response = client.update_secret(
                SecretId=secret_name,
...
# %% Deploy accounts
async def deploy_accounts(num_accounts, amount, private_key, _class_hash, secret_name_prefix="oz_account"):
    ...
    for i in range(num_accounts):
        ...
        address, tx, artifact = await deploy_starknet_account(class_hash=int(class_hash, 16), private_key=private_key, amount=amount)
...
if __name__ == "__main__":
    ...
    accounts_data = asyncio.run(deploy_accounts(num_accounts, amount, private_key, class_hash))
