...
namespace ExecutionContext {
    ...
    func get_stack{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (number_of_steps) = ExecutionContext.get_number_of_steps{syscall_ptr=syscall_ptr}(self);
        let has_steps = is_le(1, number_of_steps);
        if (has_steps == TRUE) {
            let (last_step) = ExecutionContext.get_last_step{syscall_ptr=syscall_ptr}(self);
            assert stack = last_step.stack;
        } else {
            let (initial_stack) = Stack.init{syscall_ptr=syscall_ptr}();
