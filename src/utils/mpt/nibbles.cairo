...
namespace NibblesImpl {
    ...
    func from_bytes{range_check_ptr}(bytes_len:felt, bytes: felt*) -> Nibbles {
        ...
        if (nibbles_len == 0) {
            let res = Nibbles(nibbles_len, output);
            return res;
        }
        ...
        tempvar res = Nibbles(nibbles_len, nibbles);
        return res;
    }
    ...
    func encode_path_leaf{range_check_ptr}(self: Nibbles*, is_leaf: felt) -> (bytes_len:felt, bytes: felt*){
        alloc_locals;
        let (encoded) = alloc();
