...
func slice_bytes_loop_translator{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr, bitwise_ptr: BitwiseBuiltin*
}(code_offset: felt, code_len: felt, code: felt*, new_array_len: felt, new_array: felt*) {
    alloc_locals;
...
func get_uint256_in_array{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr, bitwise_ptr: BitwiseBuiltin*
}(offset: felt, code_len: felt, code: felt*, len: felt) -> Uint256 {
    alloc_locals;
    if (len == 32) {
        // construct high
        let high: felt = get_felt_in_array_with_len(
            offset=offset, code_len=code_len, code=code, len=16
        );
        // construct low
        let low: felt = get_felt_in_array_with_len(
            offset=offset + 16, code_len=code_len, code=code, len=16
        );
        ...
        let final_data: felt = get_felt_in_array_with_len(
            offset=offset, code_len=code_len, code=code, len=len
        );
...
func get_felt_in_array_with_len{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr, bitwise_ptr: BitwiseBuiltin*
}(offset: felt, code_len: felt, code: felt*, len: felt) -> felt {
    ...
    if (res == 1) {
        ...
        let data1: felt = get_byte_in_array(
            offset=offset_rem,
            felt_packed_code=[code + offset_res],
            return_byte_length=(31 - offset_rem),
        );
        ...
        let data2_with_mask: felt = get_byte_in_array(
            offset=0, felt_packed_code=[code + offset_res + 1], return_byte_length=rem
        );
        let final_data: felt = data1_with_mask + data2_with_mask;
        // let final_uint256 : Uint256 = Uint256(final_data);
        return (final_data);
    } else {
        let final_data: felt = get_byte_in_array(
            offset=offset_rem, felt_packed_code=[code + offset_res], return_byte_length=len
        );
...
func pack_array_bytes{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr, bitwise_ptr: BitwiseBuiltin*
}(code_len: felt, code: felt*, new_array: felt*, len: felt) {
    alloc_locals;

    // %{ 
    //     import logging
    //     logging.info("PACK ARRAY BYTES")
    //     logging.info("LEN")
    //     logging.info(ids.len)
    //     logging.info("CODE LEN")
