...
namespace ModExpHelpers {
    ...
    /// Loop is implemented via uint256_modexp_recursive_call() function.
    func uint256_modexp{range_check_ptr: felt}(x: Uint256, y: Uint256, p: Uint256) -> (
        remainder: Uint256
    ) {
        alloc_locals;
        let res = Uint256(low=1, high=0);
        let (r_x, r_y, r_res) = uint256_modexp_recursive_call(x, y, res, p);
        let (quotient, remainder) = uint256_unsigned_div_rem(r_res, p);
        return (remainder=remainder);
    }

    func uint256_modexp_recursive_call{range_check_ptr: felt}(
        ...
        if ((is_equal_to_one) == 0) {
            ...
            return uint256_modexp_recursive_call(x=x_res_remainder, y=quotient, res=res, p=p);
        } else {
            let (
            ...
            return uint256_modexp_recursive_call(
