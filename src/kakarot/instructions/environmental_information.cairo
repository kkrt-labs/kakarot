...
namespace EnvironmentalInformation {
    ...
    func exec_calldataload{
        ...
        // If offset.high != 0, it's sure that offset > calldata_len.
        if (offset.high != 0) {
    ...
    func exec_calldatacopy{
        ...
        jmp out_of_bounds if offset.high != 0;
        slice(
            sliced_data,
            ctx.call_context.calldata_len,
            ctx.call_context.calldata,
        ...
        let memory = Memory.store_n(ctx.memory, size.low, sliced_data, dest_offset.low);
    ...
    func exec_returndatacopy{
        ...
        let (sliced_data: felt*) = alloc();
        jmp out_of_bounds if offset.high != 0;
        slice(sliced_data, ctx.return_data_len, ctx.return_data, offset.low, size.low);
        jmp mstore;
