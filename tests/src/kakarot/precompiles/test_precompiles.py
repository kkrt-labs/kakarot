...
class TestPrecompiles:
    class TestRun:
        ...
        def test__precompiles_run_should_fail(self, cairo_run, address, error_message):
            return_data, reverted = cairo_run(
        ...
        def test__precompiles_run_should_succeed(self, cairo_run, address, input_data):
            return_data, reverted = cairo_run(
    ...
    class TestIsPrecompile:
        ...
        def test__is_precompile_should_return_true_ethereum_precompiles(
            self, cairo_run, address
        ):
        ...
        def test__is_precompile_should_return_true_rollup_precompiles(
            self, cairo_run, address
        ):
            result = cairo_run("test__is_precompile", address=address)
            assert result == (
