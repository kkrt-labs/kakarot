...
class TestNodes:
    class TestLeaf:
        def test__should_encode_leaf_even(self, cairo_run):
            key = bytes.fromhex("646f")
            path_key = bytes.fromhex("20") + key  # odd leaf
            value = bytes.fromhex("76657262")
    ...
    class TestExtension:
        def test__should_encode_extension_even(self, leaf_node_bytes, cairo_run):
            key = bytes.fromhex("64")
            bytes.fromhex("00") + key
