...
class TestPrecompiles:
    class TestRun:
        async def test__run_should_return_a_stopped_execution_context(self, precompiles):
            # we choose datacopy precompile address 
            dataCopy_address = 0x4 
            await precompiles.test__run_should_return_a_stopped_execution_context(address=dataCopy_address).call()

    class TestIsPrecompile:
        async def test__is_precompile_should_return_false_when_address_is_greater_than_last_precompile(self, precompiles):
        ...
        async def test__is_precompile_should_return_true_when_address_is_lower_than_last_precompile(self, precompiles):
    ...
    class TestNotImplementedPrecompile:             
        async def test__not_implemented_precompile_should_raise_with_detailed_error_message(self, precompiles):
            ...
            with kakarot_error("Kakarot: NotImplementedPrecompile " + str(not_impl_precompile_address)):
    ...
    class TestExecPrecompile:
        async def test__exec_precompiles_should_throw_on_out_of_bounds(self, precompiles):
            # we choose an address of a non implemented precompile and we check that the Non Implemented Precompile error msg appear.         
            not_impl_precompile_address = 0x9
            with kakarot_error("Kakarot: NotImplementedPrecompile " + str(not_impl_precompile_address)):
