...
def raw_tx_samples():
    return [
        bytes.fromhex("f86e8263eb8505b4f9a92b82753094e688b84b23f322a994a53dbf8e15fa82cdb71127880c0e2d2235b8de888026a02c924804fba1a12e820afb1da9a2a9dd3d23894b908d11431eef22dd36e67ea0a072590d04f3e8846aabdddb6efe67a3881a27ab17d9d45fff60ef46a3bddd27f9"),
        bytes.fromhex("f871018302c89e808506a713e0da82520894e35bbafa0266089f95d745d348b468622805d82b876e00f6f06088e880c080a0081ba82131d62d76d2b836878d2b7949f2ce5de8387f685907226f505df95364a014d781beb05623e5e8836622bfb205127ddc9c398dd04c44a8ce1184cea9527b"),
        bytes.fromhex(""),
        bytes.fromhex("f86f830216c58506676ef5ec826b6c941cedc0f3af8f9841b0a1f5c1a4ddc6e1a1629074880101009bbb1fb5aa8026a0044e77af97e063a12b87fbcc083eae2b4b8daeaac46f967b5dcc82cfa1725192a06a9626195a8430f83676b3c1ca8037bbf5d2108161b0aaf07968a2cd442dc8ef")
    ]
...
class TestRLP:
    class TestRLPListEncode:
        async def test__encode_rlp_list_longer_55_bytes(self, rlp, data_to_list_long):
            ...
            expected_list = [prefix, *list(data_len.to_bytes((data_len.bit_length() + 7) // 8, 'big')), *data_to_list_long]
    ...
    class TestRLPDecode:
        ...
        async def test__decode_string_gt_55(self, rlp):
            ...
            decoded = await rlp.test__rlp_decode_at_index([prefix, *data_len, *string], 0).call()
            assert decoded.result.data == string
            assert decoded.result.is_list == False

        @pytest.mark.parametrize("raw_tx", raw_tx_samples())
        async def test__decode_list_le_55(self, rlp, raw_tx):
            contract_decoded = await rlp.test__rlp_decode_at_index(list(raw_tx), 0).call();
            assert contract_decoded.result.is_list == True
            decoded = decode(raw_tx)
            for i in range(0, len(decoded)):
                sub_decoded = await rlp.test__rlp_decode_at_index(contract_decoded.result.data,i).call();
