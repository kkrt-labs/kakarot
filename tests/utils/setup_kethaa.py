...
async def setup_test_env(starknet: Starknet) -> Tuple[StarknetContract, StarknetContract, bytes, int]: 
    ...
    account_class = await starknet.declare(source=os.path.join(os.path.dirname(__file__), "../../src/kakarot/accounts/eoa/account.cairo"), cairo_path=[os.path.join(os.path.dirname(__file__), "../../src")]);

    deployer = await starknet.deploy(
        source=os.path.join(os.path.dirname(__file__), "../../src/kakarot/accounts/registry/account/deployer.cairo"), 
