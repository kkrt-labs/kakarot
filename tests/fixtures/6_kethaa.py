...
async def setup_kethaa(starknet: Starknet) -> Tuple[StarknetContract, StarknetContract, bytes, int]: 
    ...
    account_class = await starknet.declare(source=os.path.join(os.path.dirname(__file__), "../../src/kakarot/accounts/eoa/aa/account.cairo"), cairo_path=[os.path.join(os.path.dirname(__file__), "../../src")]);

    deployer = await starknet.deploy(
        source=os.path.join(os.path.dirname(__file__), "../../src/kakarot/accounts/eoa/deployer/deployer.cairo"), 
